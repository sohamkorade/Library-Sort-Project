...
sorts = ['library_sort/' + str(i) for i in range(1, max_epsilon + 1)]
# sorts.append("std_sort/")
...
if not saved:
    ...
    df.to_csv("epsilons.csv")
else:
    df = pd.read_csv("epsilons.csv")
...
plt.figlegend(sorts)
plt.suptitle(
    "Time taken by Library Sort for different values of epsilon for different input distributions"
)
